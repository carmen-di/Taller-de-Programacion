...
procedure Eliminar (var v: vector; var dimL: rango3; valorInferior, valorSuperior: rango1);
  ...
  begin
    while (pos <= dimL) and (elemABuscar >= v[pos].codigoP) do
       pos:= pos + 1;
    if (pos > dimL) then BuscarPosicionDesde:= dimL  //caso que no existe, entonces borro desde el inferior
                    else BuscarPosicionDesde:= pos - 1;  //existe el valor en el vector
  end;
...
Begin
  ...
  then begin
		 ...
		 if (posSuperior = dimL) then //caso que no exista el superior NO HAGO CORRIMIENTO
			 dimL := dimL - salto
		 else begin
			 for i:= posInferior to dimL-1 do
				 v[i]:=v[i+salto]; 
			 dimL:=dimL-salto;
		 end;
	end;
